...
<head>
    ...
    <c:set var="item" value="${sessionScope.get('album')}"/>
    ...
    <div class="inner_content_container object_page">
        <div class="description_block">
            ...
            <div class="text_about">
                ...
                <div class="album_performers">
                    <span><fmt:message bundle="${lang_bundle}" key="header.performers"/>: </span>
                    <c:forEach var="performer" items="${item.getAlbumPerformers()}">
                ...
                <div class="album_creator">
                    <span><fmt:message bundle="${lang_bundle}" key="object_pages.creator"/>: <c:out
                            value="${sessionScope.get('album_creator').getUsername()}"/></span>
                </div>
                <div class="album_verification">
                    <span>
                        <c:choose>
                ...
                <c:if test="${user != null && (sessionScope.get('album_creator').getId() == user.getId()
                || user.getRoleId() != 3)}">
                    <a href="/servlet?cmd=change_album"><fmt:message bundle="${lang_bundle}"
                                                                     key="object_pages.change"/></a>
                </c:if>
                <c:if test="${user != null && user.getRoleId() != 3}">
                    <div class="object_managing">
                        <form method="post"
                              action="/servlet?cmd=change_album_status&status_id=1&item_id=${item.getId()}"><input
                                type="submit" value="<fmt:message bundle="${lang_bundle}" key="moderation.verify"/>">
                        </form>
                        <form method="post"
                              action="/servlet?cmd=change_album_status&status_id=2&item_id=${item.getId()}"><input
                                type="submit" value="<fmt:message bundle="${lang_bundle}" key="moderation.unverify"/>">
                        </form>
                        <form method="post"
                              action="/servlet?cmd=change_album_status&status_id=3&item_id=${item.getId()}"><input
        ...
        <div class="tracks_block">
            ...
            <c:forEach var="track" items="${item.getAlbumTracks()}">
                <c:if test="${track.getStatus() != 3 || (track.getStatus() == 3 && user != null && user.getRoleId() != 3)}">
                    <a href="/servlet?cmd=get_track&item_id=${track.getId()}" class="search_item item_small_cover">
                        <div class="item_part">
                            <img src="${item.getCoverImagePath()}" class="small_cover"/>
