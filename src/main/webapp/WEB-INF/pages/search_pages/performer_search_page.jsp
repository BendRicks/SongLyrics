...
<head>
    ...
    <title><fmt:message bundle="${lang_bundle}" key="title.performer_search"/></title>
...
<div class="content_body">
    <div class="inner_content_container search_page">
        <h1 class="page_h1"><fmt:message bundle="${lang_bundle}" key="title.performer_search"/></h1>
        <c:set value="/servlet?cmd=performer_search" var="form_action"/>
        <%@include file="../assets/search_menu.jsp" %>
        <a href="/servlet?cmd=create_performer"><fmt:message bundle="${lang_bundle}"
                                                             key="search.create_new_performer"/></a>
        <div class="search_result_block">
            <c:forEach var="item" items="${sessionScope.get('performers')}">
                <c:if test="${item.getStatus() != 3 || (item.getStatus() == 3 && user != null && user.getRoleId() != 3)}">
                    <a href="/servlet?cmd=get_performer&item_id=${item.getId()}" class="search_item item_big_cover">
                        <div class="item_part">
        ...
        <div class="pages">
            ...
            <c:forEach begin="1" end="${sessionScope.get('pages')}" var="i">
                <c:choose>
                    <c:when test="${i != page}">
                        <a href="/servlet?cmd=turn_performer_search_page&page=${i}"><c:out value="${i}"/></a>
                    </c:when>
                    <c:otherwise>
